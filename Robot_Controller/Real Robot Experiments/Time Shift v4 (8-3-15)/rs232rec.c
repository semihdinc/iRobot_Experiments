/* $Revision: 1.5.4.6 $ $Date: 2009/12/22 18:55:46 $ */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  int_T	port;
  int 	k, index, n, j, count;
  int 	ini2, outputindex;
  char 	*in;
  char 	*format_rec;
  char 	c, tempChar;
  COMData tmpc;
  ...
  if (!rs232ports[port]) {
	ssSetErrorStatus(S,"RS232 Receive: chosen COM-port not initialized\n");
	return;
  }
  ...
  for (j=0;j<k;j++) {
	...
	c = (char)(tmpc & 0x00ff);
      ...
      *in='\0';
	  ...
	  ini2 = ini2 + (int)mxGetPr(FORMAT_REC_INFO)[count+1];
	  count++;
