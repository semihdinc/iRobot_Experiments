/* $Revision: 1.6.4.7 $ $Date: 2009/12/05 02:09:56 $ */
...
static int datacount(char *formatstring)
{
  ...
  n=0;
  for (i=0;i<(int)strlen(formatstring);i++) {
	if (formatstring[i]=='%') n++;
  }
  return(n);
}
...
static void mdlInitializeSizes(SimStruct *S)
{
  ...
  num_out= (int_T)mxGetPr(NUM_OUT_ARG)[0];
  ...
  i = 0;
  while (i<(int)num_out) {
    ssSetOutputPortDataType(S, i, (DTypeId)mxGetPr(DTYPE_ARG)[i]);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  int_T port;
  int h, i, k, n, received, ini, ini2, terminationNotFound;
  char *in;
  ...
  for (h=0;h<mxGetPr(FORMAT_SEND_INFO)[0];h++) {
	...
	if (mxGetPr(FORMAT_SEND_INFO)[h+1]>0) {
	  ...
	  ini = ini + (int)mxGetPr(FORMAT_SEND_INFO)[h+1];
	...
	if (mxGetPr(FORMAT_REC_INFO)[h+1] > 0) {
      ...
      ini2 = ini2 + (int)mxGetPr(FORMAT_REC_INFO)[h+1];
	}
  }
#endif
}
